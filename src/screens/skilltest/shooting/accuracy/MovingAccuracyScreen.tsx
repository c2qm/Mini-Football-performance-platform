...
import { Footprints, Droplets, FastForward } from 'lucide-react'
...
interface MovingShotAttempt { score: number }
interface MovingDrill { id: number; target: Point2D; title: string; diff: number; setNumber: number }

const GENERATE_MOVING_DRILLS = (): MovingDrill[] => {
  const drills: MovingDrill[] = []
  const zones = [ 
    { t: 'Top Right', x: 84, y: 20, d: 1.0 }, 
    { t: 'Bottom Left', x: 16, y: 82, d: 0.9 }, 
    { t: 'Center Right', x: 65, y: 55, d: 0.8 } 
  ]
  for (let i = 0; i < 20; i++) {
    const setNumber = i < 10 ? 1 : 2
    drills.push({ id: i + 1, target: {x: zones[i%3].x, y: zones[i%3].y}, title: zones[i%3].t, diff: zones[i%3].d, setNumber })
  }
  return drills
}
const DRILLS = GENERATE_MOVING_DRILLS()

export function MovingAccuracyScreen() {
  const navigate = useNavigate()
  const [currentIndex, setCurrentIndex] = useState(0)
  const [attempts, setAttempts] = useState<MovingShotAttempt[]>([])
  ...
  if (isFinished) {
    ...
    return (
      <TestResultScreen 
        testName="Moving Accuracy" 
        score={finalScore} 
        icon={<Footprints size={28} color={THEME_COLOR} />} 
        nextTestPath="/skill-test/shooting/pressure-accuracy" 
        nextTestName="Pressure Accuracy" 
      />
    )
  }
  ...
  return (
    <ScreenContainer className="scrollbar-hide flex flex-col" withBottomNavSpacing>
      <DetailHeader title="Moving Accuracy Test" onBack={() => navigate('/skill-test/shooting')} />
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', margin: '6px 0', width: '100%' }}>
        <TestInstructionCard 
          label="Instruction" 
          title="Receive, Touch, Shoot"
          subtitle={`Target: ${currentDrill.title}`}
          color={THEME_COLOR}
        />

        <TestRulesCard 
          distance="16-18 Meters (~52-59 Feet / steps)"
          color={THEME_COLOR}
          rules={[
            "Simulates game situations. Measure ~52 to 59 walking steps from goal.",
      ...
      <div style={{ marginTop: '8px' }}>
        <Button fullWidth size="lg" disabled={!selectedResult} onClick={handleConfirmShot}>
          {selectedResult ? 'Confirm Moving Shot' : 'Tap Goal'}
        </Button>
      </div>
    </ScreenContainer>
  )
